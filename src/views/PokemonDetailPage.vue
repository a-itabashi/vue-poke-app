...
const id = ref<string>(Array.isArray(parameter.id) ? parameter.id[0] : parameter.id)
...
onMounted(async () => {
  const detailPageItem: DetailPokemon | undefined = await fetchDetailPageItem(id.value)
  if (!detailPageItem) {
    name.value = '????'
    image.value = '????'
    genera.value = '????'
    ...
    <v-card class="mx-auto" max-width="80vw" elevation="3">
      <v-sheet width="70vw">
        <v-row>
          <v-col cols="6">
            <!-- No. -->
            <div id="card_subtitle" class="mt-6 ml-9 font-weight-bold">No.{{ id }}</div>
