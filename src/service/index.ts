...
// 一覧画面表示用データの取得
const fetchListPageItem = async (_nextUrl?: string): Promise<ListPageItem | undefined> => {
  const result = _nextUrl ? await fetchPokemonList(_nextUrl) : await fetchPokemonList()
...
// 詳細画面表示用データの取得
const fetchDetailPageItem = async (id: string): Promise<DetailPokemon | undefined> => {
