...
import { self as PokemonSpecies, fetchName, fetchGenera } from '@/service/species'
...
describe('index.ts', () => {
  ...
  describe('fetchListPageItem', () => {
    ...
    describe('正常系', () => {
      beforeEach(() => {
        vi.mocked(fetchPokemonList).mockResolvedValue(mockFetchListPageItemResponse)
      ...
      test('ポケモンのリストと次のURLを要素に持つオブジェクトを返却していること', async () => {
        ...
        const mockProcessedPokemonList = Array(20).fill(mockProcessedPokemonItem)
  ...
  describe('fetchDetailPageItem', () => {
    ...
    const mockPokemonBaseInfo: Pokemon = {
      ...
      abilities: [{ ability: { url: 'testUrl1' } }, { ability: { url: 'testUrl2' } }],
    ...
    describe('正常系', () => {
      beforeEach(() => {
        ...
        vi.mocked(PokemonSpecies.fetchPokemonSpecies).mockResolvedValue(mockPokemonSpecies)
        vi.mocked(fetchName).mockResolvedValue(mockPokemonName)
    ...
    describe('異常系', () => {
      ...
      test('fetchPokemonSpecies関数でundefinedを受け取った場合、undefinedを返却すること', async () => {
        vi.mocked(Pokemon.fetchPokemon).mockResolvedValue(mockPokemonBaseInfo)
        vi.mocked(PokemonSpecies.fetchPokemonSpecies).mockResolvedValue(undefined)
        const result = await fetchDetailPageItem(id)
        expect(PokemonSpecies.fetchPokemonSpecies).toHaveBeenCalledTimes(1)
        expect(result).toBeUndefined()
      })
